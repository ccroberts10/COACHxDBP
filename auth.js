...
async function requestMagicLink(email, { name = null } = {}) {
  ...
  if (!user) {
    user = await db.one(
      `INSERT INTO users (email, name, subscription_status) VALUES ($1, $2, 'pending') RETURNING *`,
      [cleanEmail, name]
    );
    console.log('[auth] Created user:', cleanEmail);
  }
...
async function verifyMagicLink(token, { ip = null, userAgent = null } = {}) {
  ...
  await db.query(
    `INSERT INTO sessions (token, user_id, expires_at, ip_address, user_agent)
     VALUES ($1, $2, $3, $4, $5)`,
    [sessionToken, auth.user_id, sessionExpires, ip, userAgent]
  );
...
// Require Elite tier specifically (for perks endpoints)
function requireElite(req, res, next) {
  if (!req.user) return res.status(401).json({ error: 'Not logged in' });
  if (req.user.subscription_tier !== 'elite') {
