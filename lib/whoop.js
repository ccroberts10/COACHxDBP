...
// In-memory lock to prevent parallel refresh attempts for the same user.
// When 4 parallel API calls hit an expired token simultaneously, all 4 would
// try to refresh. Only the first succeeds (WHOOP rotates refresh_token on each
// use); the rest send the now-invalid token and get rejected. This caches the
// in-flight refresh promise so all parallel callers await the same refresh.
const refreshLocks = new Map();

async function refreshTokens(userId) {
  // If a refresh is already in flight for this user, return that promise
  ...
  const refreshPromise = (async () => {
    try {
      ...
      const body = new URLSearchParams({
        ...
        client_id: CLIENT_ID,
        client_secret: CLIENT_SECRET,
        scope: 'offline',  // WHOOP requires only 'offline' scope on refresh, not the full list
      });
      ...
      if (!data.access_token) {
        ...
        // Only mark integration as 'error' if WHOOP says the grant is permanently invalid
        // (not transient errors that could be network/parallel-call related).
        const errorCode = data.error || '';
        const isPermanent = errorCode === 'invalid_grant' || errorCode === 'unauthorized_client';
        if (isPermanent) {
          await db.query(
      ...
      console.log(`[whoop refresh] success for user ${userId}`);
