...
GARMIN MANUAL METRICS (garmin_sleep_score and garmin_body_battery in check-in data):
When user has self-entered Garmin metrics, treat them as high-quality recovery signals on par with WHOOP:
- garmin_sleep_score: 0-100 scale. >85 = excellent sleep, 70-85 = good, 50-70 = fair, <50 = poor (treat poor like sleep_quality 1-2)
- garmin_body_battery: 0-100 scale. >75 = high recovery (treat like WHOOP recovery >67%), 50-75 = moderate, 25-50 = low, <25 = depleted (recovery work only)
- If the user has Garmin metrics but no WHOOP, use garmin_body_battery as the primary recovery signal in Strava-only mode (overrides the inferred score below)
- In recovery_readout, mention the Garmin metric explicitly when it's the primary signal: "Body Battery 78 (high) — green light for tempo work" or "Sleep score 52 (poor) — bias easy today"
- If both WHOOP and Garmin are present (rare — user wears both), they should agree. If they disagree by more than 30 points, note the discrepancy and bias toward the more conservative signal.
...
async function buildContext(userId, whoop, strava, weather) {
  ...
  // Today's check-in (alcohol, sleep quality, legs, stress, Garmin metrics if user is on Garmin)
  const todayCheckin = await db.one(
    `SELECT sleep_quality, legs_feel, alcohol_drinks, stress_level, note,
  ...
  const recentCheckins = await db.many(
    `SELECT date, sleep_quality, legs_feel, alcohol_drinks, stress_level,
            garmin_sleep_score, garmin_body_battery
     FROM daily_checkins
