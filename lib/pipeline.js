...
// Daily pipeline runs per user: pull data → build context → call coach → save prescription
// Email "Coach is ready" reminder is sent separately by 6:30 AM cron (sendDailyReminders)
// Prescription itself is delivered in-app, not by email
...
async function runDailyForUser(userId) {
  ...
  try {
    ...
    // Process recent activities for auto-trigger rewards (big ride, etc.) — non-fatal
    try {
      const autoTriggers = require('./auto-triggers');
      await autoTriggers.processUnprocessedActivities();
    } catch (e) {
      console.log('[pipeline] auto-triggers failed (non-fatal):', e.message);
    }
    ...
    // Mark prescription as delivered (in-app only — email reminder happens separately at 6:30 AM cron)
...
// Send daily reminder emails to Coach users (no AI generation — just a wake-up nudge)
// Generation happens on-demand when user opens the app
async function sendDailyReminders() {
  const users = await db.many(
    `SELECT id, email, name FROM users
     WHERE subscription_status IN ('active', 'trialing')
       AND subscription_tier = 'coach'
       AND onboarding_completed = TRUE`
  );
  console.log(`[reminder] Sending to ${users.length} Coach users`);

  const { sendCoachReadyEmail } = require('./email');
  let sent = 0, failed = 0;
  for (const user of users) {
    try {
      await sendCoachReadyEmail(user.email, user.name);
      sent++;
    } catch (e) {
      console.error(`[reminder] Failed for ${user.email}:`, e.message);
      failed++;
    }
    // Stagger 200ms between sends
...
module.exports = { runDailyForUser, runDailyForAllUsers, sendDailyReminders };
