...
async function requestMagicLink(email, { name = null, tier = null } = {}) {
  ...
  if (!user) {
    isNew = true;
    // Free-tier signups: tier='free', status='active' (no Stripe needed)
    // Paid-tier signups (via billing/success): tier set later via webhook, status='pending' until webhook
    ...
    // Existing user with no tier signing up as free — promote them
    await db.query(
