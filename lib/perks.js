...
// ===================================================================
// STAFF REDEEM — issue + immediately redeem in one step
// Used by barista app when staff issues a perk in person at point of service.
// Skips the email-and-code roundtrip used for customer-facing rewards.
// ===================================================================
async function staffRedeemFromTemplate(userId, templateId, staffName, opts = {}) {
  const { note = null } = opts;

  // Cap check
  const cap = await checkPerkCap(userId, templateId);
  if (!cap.allowed) {
    return { success: false, error: cap.reason, capInfo: cap };
  }
  ...
  // Insert redemption already marked as redeemed
  await db.query(
    `INSERT INTO perk_redemptions
       (user_id, perk_type, code, description, expires_at,
        template_id, savings_cents, cost_cents, trigger_type, issued_by,
        redeemed_at, redeemed_by_staff, redemption_note)
     VALUES ($1, $2, $3, $4, NULL, $5, $6, $7, 'staff_inline', $8,
             NOW(), $8, $9)`,
    [userId, perkType, code, template.description,
     template.id,
     template.is_percentage ? null : template.default_retail_cents,
     template.is_percentage ? null : template.default_cost_cents,
     staffName, note]
  ...
  // For service-category perks, also log a $0 service purchase entry so the service log stays accurate
  if (template.category === 'service') {
    try {
      const memberRewards = require('./member-rewards');
      await memberRewards.logPurchase({
        userId,
        amountCents: 0,
        category: 'service',
        subcategory: template.name,
        notes: `Free perk: ${template.name}${note ? ' — ' + note : ''}`,
        staffName,
      });
    } catch (e) {
      console.warn('[perks] logPurchase for service perk failed (non-fatal):', e.message);
    }
  }

  // Get refreshed cap info for response
  const updatedCap = await checkPerkCap(userId, templateId);

  return {
    success: true,
    perk: { code, name: template.name, description: template.description },
    used: updatedCap.used,
