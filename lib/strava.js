...
// Same race condition pattern as WHOOP — parallel API calls trying to refresh
// simultaneously can invalidate each other's tokens.
  ...
  const refreshPromise = (async () => {
    try {
      ...
      if (!data.access_token) {
        // Only mark error on permanent failures, not transient
        const isPermanent = data.errors?.some(e => e.code === 'invalid' || e.code === 'unauthorized');
        if (isPermanent) {
          await db.query(
